  ...
  while (!worklist.empty()) {
    ...
    for (HloInstruction* user : changed_instr_to_propagate->users()) {
      ...
      if (user->opcode() == HloOpcode::kGetTupleElement) {
        ...
        int64_t use_index = user->operand_index(changed_instr_to_propagate);
        // Only handle one pattern at a time. If already seen a pattern do
        // nothing.
        if (instruction_handled) {
          continue;
        }
        instruction_handled = unstacker.HandleInstruction(user, use_index);
        if (!instruction_handled) {
...
// }
const HloInstruction* IsDynamicUpdateSlicingFusion(
  ...
  VLOG(3) << "IsDynamicUpdateSlicingFusion: " << instr->ToString();
  if (!metadata.unrollable_loop_bodies.contains(instr->parent())) {
    VLOG(5) << "Instruction not inside unrollable while body, "
            << instr->ToString() << instr->parent()->ToString();
    return nullptr;
    ...
    if (dynamic_index.has_value() && dynamic_index.value() == 0) {
      if (Match(fused_instr->operand(1), match::Bitcast())) {
        if (fused_instr->parent()->root_instruction() == fused_instr) {
          std::cout << "FARZIN_FOUND IsDynamicUpdateSlicingFusion" << std::endl;
          return instr;
        }
      }
    }
...
absl::Status UnstackDynamicUpdateSlicingFusion(
  ...
  // TODO: automatically find the input, offset and update indices.
...
const HloInstruction* IsDSAndDUS(const UnstackerMetadata& metadata,
                                 const HloInstruction* instr,
                                 int64_t stacked_operand_idx) {
  if (instr->opcode() != HloOpcode::kFusion) {
    return nullptr;
  ...
  if (!IsDynamicSlicingFusion(metadata, stacked->users()[0],
                              stacked->users()[0]->operand_index(stacked))) {
    return nullptr;
  }
  if (!IsDynamicUpdateSlicingFusion(
          metadata, stacked->users()[1],
          stacked->users()[1]->operand_index(stacked))) {
    return nullptr;
  }
  return stacked->users()[0];
}
...
absl::StatusOr<bool> HloUnstacker::Run(
  ...
  metadata.custom_handlers.push_back(
      std::make_pair(IsDSAndDUS, UnstackDSAndDUS));
  metadata.custom_handlers.push_back(
