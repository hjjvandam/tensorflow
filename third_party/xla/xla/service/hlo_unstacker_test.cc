...
TEST_F(UnstackerTest, UnstackSingleLoopOnlyWithDSAndDUS) {
  ...
  ENTRY SimpleLoop {
    ...
    while = (s32[], s32[4,3]) while(tuple.1), condition=
  ...
  EXPECT_TRUE(HloUnstacker().Run(module.get()).value());

  VLOG(3) << "After unstacking:\n" << module->ToString();
  std::cout << module->ToString() << std::endl;

  // After unstacking, the loop state at location 1 and 2 are unstacked,
  // therefore have tuple shape.
  // EXPECT_TRUE(module->GetComputationWithName("SimpleLoop.body")
  ...
  // // We expect the shape of the loop state at location 1 and 2 to be (s32[3],
  // // s32[3], s32[3], s32[3])
  ...
  //                 ->parameter_instruction(0)
  //                 ->shape()
  //                 .tuple_shapes(1) == expected_tuple_shape);
  // EXPECT_TRUE(module->GetComputationWithName("SimpleLoop.body")
  //                 ->parameter_instruction(0)
  //                 ->shape()
  //                 .tuple_shapes(2) == expected_tuple_shape);
}
