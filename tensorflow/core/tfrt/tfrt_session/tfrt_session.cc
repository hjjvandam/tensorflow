...
class TfrtSession : public tensorflow::Session {
 public:
  ...
  explicit TfrtSession(const SessionOptions& options,
                       ...
                       StaticDeviceMgr* device_manager)
      : runtime_{runtime},
  ...
  Status CreateLocked(GraphDef graph)
    ...
    TF_ASSIGN_OR_RETURN(
        ...
        tensorflow::tfrt_stub::FallbackState::CreateWithDeviceMgr(
            session_options, fdef_lib, device_manager_));
 ...
 private:
  ...
  StaticDeviceMgr* device_manager_ = nullptr;
};
...
Status TfrtSessionFactory::InitializeLocked(
    const TfrtSessionOptions& options, const SessionOptions& session_options) {
  mutex_.AssertHeld();
  if (options.use_tpu) {
  ...
  device_manager_ = new StaticDeviceMgr(std::move(devices));
...
Status TfrtSessionFactory::NewSession(const SessionOptions& options,
  ...
  *out_session = nullptr;
  ...
  absl::MutexLock lock(&mutex_);
  if (!IsInitialized()) {
    TF_RETURN_IF_ERROR(InitializeLocked({}, options));
  ...
  *out_session =
      new TfrtSession(options, runtime_, device_target_, tpu_use_tpu_runner_,
                      ...
                      backend_compiler, device_manager_);
  return absl::OkStatus();
}
